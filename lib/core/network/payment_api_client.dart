...
class PaymentApiClient {
  ...
  PaymentApiClient({@Named('paymentBackendUrl') required this.baseUrl, required this.dio}) {
  ...
  Future<String> createPaymentIntent(double amount, String currency) async {
    try {
      ...
      final response = await dio.post( // Use dio.post
        '/create-payment-intent', // Relative path as base URL is set in Dio options
        data: { // Dio uses 'data' for the request body
          'amount': amount, // Send amount as double, backend will convert to cents
          'currency': currency,
        },
        options: Options( // Set headers using Dio's Options
          headers: {
      ...
      if (response.statusCode == 200) {
        final Map<String, dynamic> responseData = response.data as Map<String, dynamic>; // Dio already parses JSON
        ...
        throw PaymentException('Failed to create payment intent on backend with status code ${response.statusCode}');
      }
      ...
      if (e.response != null) {
        ...
        throw PaymentException(errorData['error'] ?? 'Failed to create payment intent: ${e.message}');
      } else {
        // Handle network errors (e.g., no internet, connection refused)
        throw PaymentException('Could not connect to payment backend: ${e.message}');
      }
    } catch (e) {
      throw PaymentException('An unexpected error occurred while creating payment intent: ${e.toString()}');
    }
  }
}