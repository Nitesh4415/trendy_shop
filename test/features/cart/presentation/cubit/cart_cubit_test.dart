...
void main() {
  ...
  final testAppUser = app_user.User(
    id: 1,
    email: testEmail,
    username: 'test',
    password: 'password',
  ...
  setUp(() {
    ...
    when(
      mockAuthCubit.state,
    ).thenReturn(AuthInitial()); // Default initial state
  ...
  group('CartCubit', () {
    ...
    group('Authentication State Changes', () {
      blocTest<CartCubit, CartState>(
        ...
        setUp: () {
          when(
            mockGetCartItemsUseCase.call(testEmail),
          ).thenAnswer((_) async => [cartItem1]);
        },
    ...
    group('addItemToCart', () {
      blocTest<CartCubit, CartState>(
        ...
        setUp: () {
          when(
            mockAddToCartUseCase.call(cartItem1, testEmail),
          ).thenAnswer((_) async {});
        },
        build: () => createCubit(initialAuthState: authenticatedState),
      ...
      blocTest<CartCubit, CartState>(
        ...
        setUp: () {
          ...
          when(
            mockUpdateCartItemQuantityUseCase.call(any, any, any),
          ).thenAnswer((_) async {});
        },
        build: () => createCubit(initialAuthState: authenticatedState),
        seed: () => CartLoaded(items: [cartItem1]),
        act: (cubit) =>
            cubit.addItemToCart(cartItem1), // Add the same item again
        expect: () => [
        ...
        verify: (_) {
          ...
          verify(
            mockUpdateCartItemQuantityUseCase.call(cartItem1.id, 2, testEmail),
          ).called(1);
        },
      );
      ...
      blocTest<CartCubit, CartState>(
        ...
        act: (cubit) => cubit.addItemToCart(cartItem1),
        expect: () => [
          const CartError("User not logged in. Cannot modify cart."),
        ],
      );
    });
    ...
    group('removeItemFromCart', () {
      blocTest<CartCubit, CartState>(
        ...
        setUp: () {
          when(
            mockRemoveFromCartUseCase.call(cartItem1.id, testEmail),
          ).thenAnswer((_) async {});
        },
        build: () => createCubit(initialAuthState: authenticatedState),
    ...
    group('decrementQuantity', () {
      blocTest<CartCubit, CartState>(
        ...
        setUp: () {
          when(
            mockUpdateCartItemQuantityUseCase.call(cartItem2.id, 1, testEmail),
          ).thenAnswer((_) async {});
        },
        build: () => createCubit(initialAuthState: authenticatedState),
    ...
    group('clearAllItems', () {
      blocTest<CartCubit, CartState>(
        ...
        expect: () => [CartLoading(), const CartLoaded(items: [])],
        verify: (_) {
            ...
            .setMockMethodCallHandler(
              ...
              (MethodCall methodCall) async {
                if (methodCall.method == 'initPaymentSheet' ||
                    methodCall.method == 'presentPaymentSheet') {
                  return {};
                }
                return null;
              },
